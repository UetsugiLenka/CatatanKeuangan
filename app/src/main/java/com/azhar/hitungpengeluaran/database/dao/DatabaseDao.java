...
public interface DatabaseDao {
    ...
    @Query("SELECT COALESCE((SELECT SUM(jml_uang) FROM tbl_keuangan WHERE tipe = 'pemasukan'), 0) - COALESCE((SELECT SUM(jml_uang) FROM tbl_keuangan WHERE tipe = 'pengeluaran'), 0)")
    LiveData<Integer> calculateSelisih();
